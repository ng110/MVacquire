...
#	endif // DOXYGEN_CPP_DOCUMENTATION
	/// \brief A constant defining that a new component will be inserted at the end of a list when it's registered.
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief One or more of the specified parameters are not supported
	/// by the function.
	...
	/// \brief It has been tried to register the same component at twice
	/// in the same list.
	...
	/// \brief This indicates an internal error occurred within the SWIG generated
	/// wrapper code, when working under Python.
	...
	/// \brief The user tried to modify a registered callback, but no callback has been
	/// registered for this component.
	PROPHANDLING_COMPONENT_NO_CALLBACK_REGISTERED = -2030,
	/// \brief The user tried to read data into a user supplied storage location, but the 
	/// buffer was too small to accommodate the result.
	...
	/// \brief The user tried to execute an operation, which is not supported by the
	/// component he is referring to.
	PROPHANDLING_UNSUPPORTED_OPERATION = -2033,
	/// \brief The user tried to save(serialize) a property list without having the right to
	/// do this.
	PROPHANDLING_CANT_SERIALIZE_DATA = -2034,
	/// \brief The user tried to use a file to update or create a component list, that does not
	/// contain valid data for this operation.
	///
	...
	/// \brief This error will occur when the modules internal representation of the tree structure
	/// does not allow the allocation of a new list.
	///
