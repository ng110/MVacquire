...
#	ifndef MV_CUSTOM_IMAGE_BUFFER_PIXEL_FORMATS
#		define MV_CUSTOM_IMAGE_BUFFER_PIXEL_FORMATS
#	endif
#	ifndef MV_CUSTOM_IMAGE_DESTINATION_PIXEL_FORMATS
#		define MV_CUSTOM_IMAGE_DESTINATION_PIXEL_FORMATS
#	endif
...
//-----------------------------------------------------------------------------
{
	...
	///
	/// \note This feature might change in future releases! Use it with care and 
	/// be ready to change your code later.
	enum TAcquisitionMode
	//-----------------------------------------------------------------------------
	{
		/// \brief Continuous mode. This is the recommended mode when image data shall either
		/// be transferred constantly or when working with an externally triggered setup.
		amContinuous = 1,
		...
		/// to send more data
		amMultiFrame = 2,
		/// \brief In this mode the device always will always just send a single image when a data stream 
		/// is started.
		///
...
///
/// Auto control speed modes define the time in which the controller tries to adapt it's parameters to
...
//-----------------------------------------------------------------------------
{
	...
	///
	/// This mode is fast but produces a larger amount of artefacts. Especially sharp edges will
	/// appear slightly blurred in the resulting image.
	bcmLinearInterpolation,
	/// \brief Adaptive edge sensing.
	///
	/// This mode requires more CPU time then linear interpolation, but the resulting image more
	/// closely matches the original scene. Edges will be reconstructed with higher accuracy.
...
//-----------------------------------------------------------------------------
{
	/// \brief It is not known whether the buffer or image contains raw Bayer data or the
	/// buffer or image does NOT contain raw Bayer data.
	bmpUndefined = -1,
...
//-----------------------------------------------------------------------------
{
	...
	bwbrUnknown = 0,
	/// \brief The white balance calibration has been performed successfully for the
	/// selected setting.
	bwbrOK = 1,
	/// \brief An unknown error occurred during the white balance calibration for the
	/// selected setting.
	bwbrErrorUnknown = 2,
	/// \brief The previous white balance calibration failed because the reference image
	/// used for the calibration was too dark.
	bwbrErrorTooDark = 3,
	/// \brief The previous white balance calibration failed because the reference image
	/// used for the calibration was too bright.
	...
	//-----------------------------------------------------------------------------
	{
		/// \brief This input is assumed as logic '1' when the voltage applied to the
		/// input lies above about 2 Volts.
		bfdit2V = 2,
		/// \brief This input is assumed as logic '1' when the voltage applied to the
		/// input lies above about 10 Volts.
		bfdit10V = 10
	};
	...
	//-----------------------------------------------------------------------------
	/// \brief Defines the version of the kernel driver the mvBlueFOX is running with.
	...
	//-----------------------------------------------------------------------------
	/// \brief Defines the block size used for the USB transfer of the mvBlueFOX.
...
//-----------------------------------------------------------------------------
{
	...
	/// work like in <b>mvIMPACT::acquire::cemStandard</b> but only a short pulse will be send to
	/// the output.
	cemOverlapped,
...
#ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
	//-----------------------------------------------------------------------------
	/// \brief Some grabbers are able to get the video signal and sync signals ( HD-IN, VD-IN ) on separate lines.
	/// The user can define this for non standard cameras.
	...
	//-----------------------------------------------------------------------------
	/// \brief Some grabbers are able to get the video signal and the sync signal on different input pins.
	/// The user can select this for non standard cameras.
...
//-----------------------------------------------------------------------------
{
	/// \brief Flash signal is on while the shutter is open on a frame shutter sensor or 
	/// during the complete exposure period of a rolling shutter sensor.
	cftStandard = 0,
	/// \brief Flash signal is only on during the time all lines of a rolling shutter sensor are 
	/// exposed. 
	///
...
//-----------------------------------------------------------------------------
{
	/// \brief The video signal is transmitted non interlaced, meaning that a complete
	/// image is transferred without interruption.
	citNone,
...
#ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
	//-----------------------------------------------------------------------------
	/// \brief Multitap cameras can transmit data in different ways. This enum can be used to
	/// define how the camera transmits a line of pixels.
	...
	//-----------------------------------------------------------------------------
	{
		...
		/// </table>
		cltxg4X = 8,
		/// \brief Four taps. The first two taps will transmit the first half of the line, while the second two taps will transmit the second half of the line.
		...
		/// </table>
		cltxg8X = 14,
		/// \brief Eight taps. Every two taps will transmit one quarter of the line.
		/// Sorting of data needed.
		///
		...
		/// <tr><td class="indexvalue">2</td><td class="indexvalue">3</td><td class="indexvalue">5</td><td class="indexvalue">W/4+3</td><td class="indexvalue">W/4+4</td><td class="indexvalue">W/2+3</td><td class="indexvalue">W/2+4</td><td class="indexvalue">3W/4+3</td><td class="indexvalue">3W/4+4</td></tr>
		/// <tr><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td><td class="indexvalue">...</td></tr>
		/// <tr><td class="indexvalue">W/8</td><td class="indexvalue">W/4-1</td><td class="indexvalue">W/4</td><td class="indexvalue">W/4</td><td class="indexvalue">W/2</td><td class="indexvalue">3W/4-1</td><td class="indexvalue">3W/4</td><td class="indexvalue">W-1</td><td class="indexvalue">W</td></tr>
		/// </table>
		cltxg4X2 = 15
	};
...
//-----------------------------------------------------------------------------
{
	/// \brief Don't wait for trigger. In this mode the camera continuously exposes images with
...
//-----------------------------------------------------------------------------
{
	...
	//-----------------------------------------------------------------------------
	{
		/// \brief Clamp pulse starts at 80 % of ActiveVideoAoi.X .
		ecmAuto, 
		/// \brief The user can set the clamp start time in micro sec. after the falling edge of the 
		/// video signals' VSync.
		ecmUser
	};
...
//-----------------------------------------------------------------------------
{
	/// \brief The driver decides (depending on the connected camera) what kind of
	/// colour processing has to be applied.
	cpmAuto = 0,
	/// \brief No colour processing will be performed.
	cpmRaw,
	/// \brief A Bayer colour conversion will be applied before the image is transferred
	/// to the user.
	cpmBayer,
	/// \brief A Bayer to mono conversion will be applied before the image is transferred
	/// to the user.
	cpmBayerToMono,
	/// \brief No colour processing will be performed but the packed raw Bayer data will be re-arranged
	/// within the buffer.
	///
...
//-----------------------------------------------------------------------------
{
	...
	dcHotplugable = 0x1,
	/// \brief This is a device, that has more then one video input channel.
...
//-----------------------------------------------------------------------------
{
	...
	ddomUser,
	/// \brief The digital output will change its state during the active exposure period
	/// of the image sensor and will switch back to its initial state again once the exposure period is over.
	ddomExposureActive,
	...
	/// \brief When there is at least one outstanding request the digital output will change its state during the active exposure period
	/// of the image sensor and will switch back to its initial state again once the exposure period is over.
...
//-----------------------------------------------------------------------------
{
	/// \brief This event won't be signalled in this state even if the underlying
	/// event has been noticed by the device driver.
	demIgnore,
	/// \brief This event will be notified whenever the underlying event
	/// has been detected by the device driver.
	demNotify
};
...
//-----------------------------------------------------------------------------
{
	...
	/// hotplug compliant device recognized by the mvIMPACT acquire device 
	...
	/// start of a new image has been detected by the device.
	///
	/// \note
	/// This is currently only supported by mvTITAN/mvGAMMA devices.
	detFrameStart = 0x4,
	/// \brief An event of this type will be signalled (<b>if desired</b>) each time the
	/// the histogram is calculated.
	///
	/// \note
...
//-----------------------------------------------------------------------------
{
	/// \brief A generic interface layout shall be used(<b>deprecated</b>).
	///
	/// This interface layout might be available when a third party hardware is
	/// (or claims to be) compliant with a certain standard and the used device
	/// driver also implements this standard. The main purpose of this interface
	/// layout is to grant access to third party devices without having in-depth
...
//-----------------------------------------------------------------------------
{
	...
	dlsAuto = 0,
	/// \brief No stored settings will be loaded at start-up. The device will be
	/// initialized with the drivers default values.
	dlsNoLoad
	//, dlsFamily,
	//dlsProduct,
	//dlsCameraLocal
...
#ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
	//-----------------------------------------------------------------------------
	/// \brief Valid trigger modes for grabbers.
...
//-----------------------------------------------------------------------------
{
	...
	//-----------------------------------------------------------------------------
	{
		...
		docmSoftware = 0,
		/// \brief The digital output signal is generated by a <b>R</b>eal <b>T</b>ime <b>C</b>ontroller
		/// that is part of the used hardware device.
		docmRTC
	};

	//-----------------------------------------------------------------------------
	/// \brief Defines valid digital source signal constants that can be used to
	/// configure a devices behaviour in certain digital I/O related modes.
	///
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief An unknown error occurred while processing a user called
	/// driver function.
	DEV_UNKNOWN_ERROR = -2110,
	/// \brief A driver function has been called with an invalid device handle.
	DEV_HANDLE_INVALID = -2111,
	/// \brief A driver function has been called but one or more of the input
	...
	/// the value of the property \a RequestCount in the <b>SystemBase</b> list - 1.
	...
	/// \brief The user requested a new image, but no free <b>mvIMPACT::acquire::Request</b>
	/// object is available to process this request.
	...
	/// \brief A suitable driver library to work with the device manager has been detected, but
	/// it is too old to work with this version of the mvDeviceManager library.
	///
	...
	/// terminated in an unusual wait, \a DMR_WAIT_ABANDONED will be returned then.
	...
	/// \brief A request has been configured to use a user supplied buffer, but the buffer pointer associated with
	/// the request is invalid.
	DEV_REQUEST_BUFFER_INVALID = -2138,
	/// \brief A request has been configured to use a user supplied buffer, but the buffer pointer associated with
	/// the request has an incorrect alignment.
	///
	...
	///
	/// There can be multiple reasons for this error and a detailed error message will be send to the log-output
	...
	/// \brief A dummy constant to mark the last valid error code for device and device manager 
	/// related errors.
	DMR_LAST_ASSIGNED_ERROR_CODE = DMR_PSEUDO_LAST_ASSIGNED_ERROR_CODE - 2,
#endif // #if !defined(DOXYGEN_SHOULD_SKIP_THIS) && !defined(WRAP_ANY) && !defined(WRAP_DOTNET)
	/// \brief Defines the last valid error code value for device and device manager 
	/// related errors.
...
#ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
	//-----------------------------------------------------------------------------
	/// \brief The fieldgate is used to detect the field ( odd or even ) for interlaced videosignals.
	enum TFieldGateMode
	//-----------------------------------------------------------------------------
	{
...
//-----------------------------------------------------------------------------
/// \brief Defines valid <b>Device</b> HW update results.
///
...
//-----------------------------------------------------------------------------
{
	...
	/// point to Y(1) when using a byte pointer.
	ibpfYUV422Planar = 12,
	/// \brief The image will be transferred as an RGB image with 30 bit of 
	/// usable data per pixel.
	///
	...
	/// \brief The image will be transferred as an RGB image with 36 bit of 
	/// usable data per pixel.
	///
	/// This is an interleaved pixel format with 2 bytes per colour component.
	/// The data is stored pixel-wise:
	...
	/// \brief The image will be transferred as an RGB image with 42 bit of 
	/// usable data per pixel.
	///
	/// This is an interleaved pixel format with 2 bytes per colour component.
	/// The data is stored pixel-wise:
	...
	/// \brief The image will be transferred as an RGB image with 48 bit of 
	/// usable data per pixel.
	///
	/// This is an interleaved pixel format with 2 bytes per colour component.
	/// The data is stored pixel-wise:
	...
	///\code
	/// //-----------------------------------------------------------------------------
	/// inline unsigned short GetMonoPacked_V2Pixel( unsigned char* pBuffer, int pixel, int shift )
	/// //-----------------------------------------------------------------------------
	/// {
	...
	/// \brief The image will be transferred as an YUV image with 30 bit of
	/// usable data per pixel.
	///
	/// This is an interleaved pixel format with 2 bytes per colour component.
	/// The data is stored pixel-wise:
...
#if !defined(DOXYGEN_SHOULD_SKIP_THIS) && !defined(WRAP_ANY) && !defined(WRAP_DOTNET)
	, idpfLAST,
	idpfCUSTOM = 0x8000000,
	MV_CUSTOM_IMAGE_DESTINATION_PIXEL_FORMATS
#endif // #if !defined(DOXYGEN_SHOULD_SKIP_THIS) && !defined(WRAP_ANY) && !defined(WRAP_DOTNET)
};

//-----------------------------------------------------------------------------
/// \brief Defines valid filters which can be applied to the captured image before it
...
//-----------------------------------------------------------------------------
{
	...
	/// \brief Reserved. Currently not implemented.
	ircmCounting,
	/// \brief In this mode no 'real' image will be captured, but the whole 
	/// processing chain will be traversed once.
	///
...
//-----------------------------------------------------------------------------
{
	...
	///
	/// GRGRGRGRGRGRG etc.<br>
	/// BGBGBGBGBGBGB etc.<br>
	/// GRGRGRGRGRGRG etc.<br>
	/// etc.<br>
	iscpGreenRed,
	...
	/// The raw image therefore is structured like this:
	///
	/// RGRGRGRGRGRGR etc.<br>
	/// GBGBGBGBGBGBG etc.<br>
	/// RGRGRGRGRGRGR etc.<br>
	/// etc.<br>
	iscpRedGreen,
...
//-----------------------------------------------------------------------------
{
	...
	///
	/// If <b>mvIMPACT::acquire::lcOn</b> is used of 
	/// <b>mvIMPACT::acquire::RequestInfo</b> structure contains the line count
	/// of the first line of the <b>mvIMPACT::acquire::Request::infoLineCounter</b>.
	///
...
//-----------------------------------------------------------------------------
{
	...
	LUTgmStandard,
	/// \brief Maps an image by applying a linear interpolation in the lower intensity range of the LUT and an 
	/// intensity transformation with gamma correction to the upper intensity range of the LUT.
	LUTgmLinearStart
};
...
//-----------------------------------------------------------------------------
{
...
#ifndef IGNORE_MVGRABBER_SPECIFIC_DOCUMENTATION
	//-----------------------------------------------------------------------------
	/// \brief Defines valid modes for the handling of the device' on board memory
	/// (if available)
	...
	//-----------------------------------------------------------------------------
	{
		...
		obmmDefault,
		/// \brief The user can define how the on board memory of the device will
		/// be used.
		///
...
//-----------------------------------------------------------------------------
{
	...
	rrOK = 0,
	/// \brief This image request resulted in a timeout. No image has been captured during
	...
	/// \endif
	rrError = 2,
	/// \brief This request has been aborted either because there are no free internal buffers
	/// or the user itself caused this abort e.g. by clearing the request queue.
	rrRequestAborted = 3,
	...
	///
	/// This e.g. might happen with drivers that transmit buffers that contain more then a pure block of pixel
	...
	/// \brief There is not enough memory available to the driver to process the current
	/// image request.
	///
	/// To get more memory either some old requests should be unlocked or the size of the
	/// DMA memory (frame grabbers only) could be increased using the tools provided.
...
//-----------------------------------------------------------------------------
{
	...
	//-----------------------------------------------------------------------------
	{
		...
		/// <b>mvIMPACT::acquire::TCameraExternalSyncEdge::ceseFallingEdge</b> or
		/// <b>mvIMPACT::acquire::TCameraExternalSyncEdge::ceseRisingEdge</b>.
		/// Otherwise <b>mvIMPACT::acquire::TScanClock::scAnalogue</b> is used for <b>mvTITAN-RGB</b>
		/// and <b>mvTITAN-G1</b> devices and <b>mvIMPACT::acquire::TScanClock::scDigital</b> for
	...
	//-----------------------------------------------------------------------------
	{ 
		/// \brief 720 x 576 Pixels for 50 Hz standard cameras
		///		720 x 480 Pixels for 60 Hz standard cameras
		ssITU601,
		/// \brief 768 x 576 Pixels for 50 Hz standard cameras
		///		640 x 480 Pixels for 60 Hz standard cameras
		ssSquare,
...
//-----------------------------------------------------------------------------
/// \brief Defined valid values for the behaviour of the user data when a device
/// has been disconnected and reconnected within a running process.
enum TUserDataReconnectBehaviour
//-----------------------------------------------------------------------------
{
	/// \brief Keep the data currently buffered in the properties describing the
	/// user data. 
	///
	...
	udrbKeepCachedData,
	/// \brief Updates the properties describing the user data with the fresh data as
	/// read from the devices non-volatile memory.
	///
...
//-----------------------------------------------------------------------------
{
	...
	//-----------------------------------------------------------------------------
	{
		/// \brief Will capture all recognized image formats
		vditAll,
	...
	//-----------------------------------------------------------------------------
	/// \brief Defines valid test modes.
	...
	//-----------------------------------------------------------------------------
	{
		...
		vdtmMovingMonoRamp,
		/// \brief Will generate a moving test pattern with pixel format <b>mvIMPACT::acquire::ibpfYUV422Packed</b> or <b>mvIMPACT::acquire::ibpfYUV422_10Packed</b>
		/// depending on the value of the 'ChannelBitDepth' property.
		...
		/// \brief Will generate a moving test pattern with pixel format <b>mvIMPACT::acquire::ibpfYUV422_UYVYPacked</b> or <b>mvIMPACT::acquire::ibpfYUV422_UYVY_10Packed</b>
		/// depending on the value of the 'ChannelBitDepth' property.
		...
		/// \brief Will generate a moving test pattern with pixel format <b>mvIMPACT::acquire::ibpfYUV444Packed</b> or <b>mvIMPACT::acquire::ibpfYUV444_10Packed</b>
		/// depending on the value of the 'ChannelBitDepth' property.
		vdtmMovingYUV444PackedRamp,
		/// \brief Will generate a moving test pattern with pixel format <b>mvIMPACT::acquire::ibpfYUV444_UYVPacked</b> or <b>mvIMPACT::acquire::ibpfYUV444_UYV_10Packed</b>
		/// depending on the value of the 'ChannelBitDepth' property.
