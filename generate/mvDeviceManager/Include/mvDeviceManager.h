...
#ifdef DOTNET_ONLY_CODE
	int getiChannelOffset( void ) const { return iChannelOffset; }
	int getiLinePitch( void ) const { return iLinePitch; }
	int getiPixelPitch( void ) const { return iPixelPitch; }
	std::string getszChannelDesc( void ) const { return std::string(szChannelDesc); }
#endif // #ifdef DOTNET_ONLY_CODE
} ATTR_PACK;
...
//-----------------------------------------------------------------------------
{
	...
	///
	/// This value in connection with <b>mvIMPACT::acquire::ImageBuffer::vpData</b> is sufficient
	/// to copy the complete image without having any additional information about it.
	/// \sa
	/// <b>mvIMPACT::acquire::ImageBuffer::vpData</b>
	int iSize;
	/// \brief The starting address of the image.
	///
	/// This address in connection with <b>mvIMPACT::acquire::ImageBuffer::iSize</b> is sufficient
	/// to copy the complete image without having any additional information about it.
	/// 
	///\xmlonly
	...
	/// <b>EXAMPLE:</b><br><br>
	///\code
	/// const ImageBuffer* pib = getImageBuffer();
	/// char* pTempBuf = new char[ib.iSize];
	...
	/// again. However once a <b>mvIMPACT::acquire::Request</b> has been returned to the user, it's <b>mvIMPACT::acquire::ImageBuffer</b>
	...
	/// <b>mvIMPACT::acquire::ChannelData</b> structures <b>mvIMPACT::acquire::ImageBuffer::pChannels</b> is pointing to 
	/// once this structure has been allocated and filled with valid data.
	int iChannelCount;
	/// \brief A pointer to an array of channel specific image data.
	ChannelData* pChannels;
#ifdef DOTNET_ONLY_CODE
	int getiBytesPerPixel( void ) const { return iBytesPerPixel; }
	int getiHeight( void ) const { return iHeight; }
	int getiWidth( void ) const { return iWidth; }
	TImageBufferPixelFormat getPixelFormat( void ) const { return pixelFormat; }
	int getiSize( void ) const { return iSize; }
	unsigned char* getvpData( void ) const { return (unsigned char*)vpData; }
	int getiChannelCount( void ) const { return iChannelCount; }
	ChannelData* getChannel( unsigned int index ) const { return &pChannels[index]; }
#endif // #ifdef DOTNET_ONLY_CODE
} ATTR_PACK;

#if !defined(DOXYGEN_CPP_DOCUMENTATION) && !defined(WRAP_ANY)
	//-----------------------------------------------------------------------------
	/// \brief This structure contains information about the image currently associated with
	/// this <b>mvIMPACT::acquire::Request</b>.
	struct RequestInfo
	//-----------------------------------------------------------------------------
	{
		/// \brief A frame identifier used by certain <b>mvIMPACT::acquire::RTCtrProgram</b> steps.
		///
		/// This parameter is returned as part of the <b>mvIMPACT::acquire::Request</b>. It can be
		/// used to associate a certain image with a unique identifier. When an 
		/// <b>mvIMPACT::acquire::RTCtrProgramStep</b> instruction of the type <b>mvIMPACT::acquire::rtctrlProgTriggerSet</b>
		...
		/// the property.
		int frameID;
		/// \brief The number of images captured since the driver has been initialised including
		/// the current image.
		int frameNr;
		/// \brief A timestamp (in us) defining the time the device started the exposure of the
		...
		int exposeTime_us;
		/// \brief The time the transaction of this image has been delayed (in us) because
	...
	/// <b>mvIMPACT::acquire::Request</b> or the image currently referenced by it.
...
//-----------------------------------------------------------------------------
{
	/// \brief A consecutive number telling the user how many times this event
	...
	unsigned int getTimestamp_lowPart( void ) const { return timestamp_lowPart; }
	unsigned int getTimestamp_highPart( void ) const { return timestamp_highPart; }
#endif // #ifdef DOTNET_ONLY_CODE
} ATTR_PACK;
...
//-----------------------------------------------------------------------------
/// \brief Flags to define the way an <b>mvIMPACT</b> buffer are created and handled.
...
//-----------------------------------------------------------------------------
{
	...
	/// accepting flags of type <b>mvIMPACT::acquire::TImpactBufferFlag</b> it migth free the
...
typedef int HOBJ;
	...
	//-----------------------------------------------------------------------------
	{
		...
		///
		/// \note
		/// This value is only defined if the referenced object is of type <b>TComponentType::ctList</b>.
		sqListContentDescriptor	= 2,
		/// \brief The value of the object referenced by HOBJ.
		///
		/// \note
		/// This value is only defined if the referenced object is of type <b>TComponentType::ctProp</b>.
		sqPropVal				= 3,
		...
		///
		/// \note
		/// This value is only defined if the referenced object is of type <b>TComponentType::ctMeth</b>.
		///
	...
	//-----------------------------------------------------------------------------
	{
		...
		/// \brief An integer value containing the current firmware version this
	...
	//-----------------------------------------------------------------------------
	/// \brief Valid search modes for the function <b>DMR_GetDevice()</b> when searching
	/// for a certain device.
	///
	/// \note
	/// <b>TDMR_DeviceSearchMode::dmdsmUseDevID</b> can be 'ored' (|) together with 
	/// all the other modes.
	...
	{ 
		...
		///
		/// When <b>TDMR_DeviceSearchMode::dmdsmUseDevID</b> is specified, the device
